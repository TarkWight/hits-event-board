...
impl EventInvariants {
    pub fn check_time_span(starts_at: time::OffsetDateTime, ends_at: Option<time::OffsetDateTime>) -> Result<(), &'static str> {
        if let Some(e) = ends_at { if e <= starts_at { return Err("ends_at must be after starts_at"); } }
        Ok(())
    }
    pub fn check_deadline(signup_deadline: Option<time::OffsetDateTime>, starts_at: time::OffsetDateTime) -> Result<(), &'static str> {
        if let Some(d) = signup_deadline { if d > starts_at { return Err("signup_deadline must be <= starts_at"); } }
        Ok(())
    }
}
